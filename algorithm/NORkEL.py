...
from NORkELAnn import NORkELAnn


class NORkEL(nn.Module):
    def __init__(self, para_train_data: np.ndarray = None,
        ...
        @param para_density_threshold: Density threshold value.
        ...
        temp_density = self.get_label_density()

        # according the label matrix density to construct labelsets
        if temp_density > para_density_threshold:
            self.get_nearest_label_subset()  # k-random strategy
        else:
            self.get_random_label_subset()  # k-nearest strategy
        self.norkel_ann = NORkELAnn(self.train_data, self.train_target_loss,
    ...
    def get_label_density(self):
        """
        Get label matrix density

        @return: Density value
        """
        temp_label_num = np.sum(self.train_target == 1)
        density = temp_label_num / (self.train_target.size)
    ...
    def predict(self):
        ...
        """
        result_matrix = self.norkel_ann.predict()
        # 求多个网络的预测均值
    ...
    def get_random_label_subset(self):
        """
            Randomly construct the labelsets.
        """
        print("select by random")
        temp_label = self.train_target.shape[1]
        result = []
        temp_select_list = []
        temp_train_target_list = []
        temp_label_index_length = len(str(temp_label))
        for i in range(temp_label):
            temp_li = [_ for _ in range(temp_label)]
            temp_li.remove(i)
            temp_list = random.sample(temp_li, self.k_labels - 1)
            temp_list.append(i)
            temp_list = (-1 * np.sort(-1 * np.array(temp_list))).tolist()
            # 将列表中的元素转换为字符串用于去重
            temp_str = ''.join("0" * (temp_label_index_length - len(str(_))) + str(_) for _ in temp_list)
